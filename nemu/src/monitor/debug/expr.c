			...
			if(regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
				...
				switch(rules[i].token_type) {
					case NOTYPE:
						break;
					case NEHENUM:
						if(nr_token != 0 && (tokens[nr_token - 1].type == NUM || tokens[nr_token - 1].type == NENUM || tokens[nr_token - 1].type == HENUM || tokens[nr_token - 1].type == NEHENUM || tokens[nr_token - 1].type == REG)) {
